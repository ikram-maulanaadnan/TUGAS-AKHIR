...
async function getSensorReadings(limit = 50) {
    const result = await query("SELECT * FROM sensor_readings ORDER BY timestamp DESC LIMIT $1", [
        limit
    ]);
    return result.rows;
}
...
async function getAIRecommendations(limit = 10) {
    const result = await query("SELECT * FROM ai_recommendations ORDER BY created_at DESC LIMIT $1", [
        limit
    ]);
    return result.rows;
}
async function getSystemLogs(limit = 50) {
    const result = await query("SELECT * FROM system_logs ORDER BY timestamp DESC LIMIT $1", [
        limit
    ]);
    return result.rows;
}
...
"[project]/src/app/api/ai-recommendations/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const recommendations = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$db$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getAIRecommendations"])(limit);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__231e784a._.js.map